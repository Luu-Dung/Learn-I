...
#include <linux/ioctl.h> // pProvide ioctl service functions.
...
#define VCHAR_CLR_DATA_REGS _IO(MAGIC_NUMBER,0)
#define VCHAR_GET_STS_REGS _IOR(MAGIC_NUMBER,1, sts_regs_t*)
#define VCHAR_SET_RD_PER_DATA_REGS _IOW(MAGIC_NUMBER,2, unsigned char *)
#define VCHAR_SET_WR_PER_DATA_REGS _IOW(MAGIC_NUMBER,3, unsigned char *)


/****************************** Driver Structs *****************************/
// Stuct for storing value of sts registers.
typedef struct {
...
/* D2. Ham xoa gia tri data registers */
int vchar_hw_clear_data(vchar_dev_t *hw){
	...
	// Set tat ca byte cua thanh ghi data ve 0
	memset(hw->data_regs,0,NUM_DATA_REGS*REG_SIZE);

	// ko can biet  co Over hay ko. XOa cho chac an
	hw->status_regs[DEVICE_STATUS_REG] &= ~STS_DATAREGS_OVERFLOW_BIT;
...
static long vchar_driver_ioctl(struct file *filp, unsigned int cmd, unsigned long arg){
	...
	switch(cmd){
		case VCHAR_CLR_DATA_REGS:
		{
			...
			if(ret < 0){
				printk("Can't clear data registers\n");
			}
			else{
				printk("Data register has been cleared\n");
			}
		}
		break;
		...
		break;
		case VCHAR_SET_RD_PER_DATA_REGS:
		{
		...
		break;
		case VCHAR_SET_WR_PER_DATA_REGS:
		{
